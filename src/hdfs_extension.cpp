...
inline void HdfsOpenSSLVersionScalarFun(DataChunk &args, ExpressionState &state,
                                        Vector &result) {
  auto &name_vector = args.data[0];
  UnaryExecutor::Execute<string_t, string_t>(
      name_vector, result, args.size(), [&](string_t name) {
...
static void LoadInternal(DatabaseInstance &instance) {
  ...
  auto hdfs_openssl_version_scalar_function =
      ScalarFunction("hdfs_openssl_version", {LogicalType::VARCHAR},
                     LogicalType::VARCHAR, HdfsOpenSSLVersionScalarFun);
  ExtensionUtil::RegisterFunction(instance,
                                  hdfs_openssl_version_scalar_function);
}
